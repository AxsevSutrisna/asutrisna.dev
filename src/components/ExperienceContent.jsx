...
import { Briefcase, GraduationCap, MapPin, CalendarDays, Code2, CheckCircle2, Sparkles } from 'lucide-react';
import { normalizeWorkExperience, compareWorkExperienceTimeline, formatDateRange as formatWorkDate } from '../utils/workExperiences';
import { normalizeEducation, compareEducationTimeline, formatDateRange as formatEduDate } from '../utils/educations';
...
export default function ExperienceContent() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (workRes.data) {
          setWorkExperiences(workRes.data.map(normalizeWorkExperience).sort(compareWorkExperienceTimeline));
        }
        if (eduRes.data) {
          setEducations(eduRes.data.map(normalizeEducation).sort(compareEducationTimeline));
        }
      } catch (error) {
  ...
  return (
    <section className="pt-32 pb-24 px-[5%] md:px-[10%] min-h-screen">
