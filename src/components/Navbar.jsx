  ...
  return (
    <nav className="fixed w-full top-0 z-50 transition-all duration-500">
      <div className="mx-auto px-[5%] sm:px-[5%] lg:px-[10%] pt-4">
        <div
          className={`flex items-center justify-between h-16 px-4 sm:px-6 rounded-full border transition-all duration-300 ${scrolled || isOpen ? "bg-[#0a0a1a]/90 border-white/10 backdrop-blur-md shadow-lg" : "bg-[#0a0a1a] border-white/10 shadow-[0_4px_30px_rgba(0,0,0,0.1)]"}`}
        >
              ...
              {navItems.map((item) => {
                ...
                return (
                  <MenubarMenu key={item.label}>
                    <MenubarTrigger
                      asChild
                      className={`cursor-pointer rounded-full px-4 py-2 text-sm font-medium transition-all duration-200 cursor-target ${
                        isActive
          ...
          {navItems.map((item, index) => {
            ...
            return (
              <Button
                ...
                asChild
                variant="ghost"
                className={`w-full justify-start rounded-xl px-4 py-4 text-base font-medium transition-all duration-300 ease border cursor-target ${
                  isActive
