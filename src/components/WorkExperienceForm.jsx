...
function WorkExperienceForm({
  ...
  const initialForm = useMemo(() => {
    ...
    return {
      ...
      end_month: normalized.is_current
        ? ""
        : normalized.end_month
          ? String(normalized.end_month)
          : "",
      end_year: normalized.is_current
        ? ""
        : normalized.end_year
          ? String(normalized.end_year)
          : "",
  ...
  const activeErrors = Object.keys(errors).length > 0 ? errors : localErrors

  const set = (key) => (event) => {
    const value =
      key === "is_current" ? event.target.checked : event.target.value
  ...
  return (
    <form
      onSubmit={(event) => {
        ...
        const nextErrors = validateWorkExperienceForm(form)
        ...
        onSubmit(buildWorkExperiencePayload(form))
      }}
      noValidate
    ...
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <SelectField
          ...
          error={activeErrors.employment_type}
        />
        ...
        <div className="sm:col-span-2">
          <MonthYearSelect
            ...
            errorMonth={activeErrors.start_month}
            errorYear={activeErrors.start_year}
          />
        </div>
        ...
        {!form.is_current && (
          <div className="sm:col-span-2">
            <MonthYearSelect
              ...
              errorMonth={activeErrors.end_month}
              errorYear={activeErrors.end_year}
            />
            <FieldError message={activeErrors.end_date} />
          </div>
        )}
        ...
        <div className="sm:col-span-2 space-y-1.5">
          ...
          <textarea
            ...
            className={`w-full bg-[#0d0d22] border rounded-xl px-4 py-2.5 text-gray-200 placeholder-gray-600 text-sm outline-none transition-all resize-none focus:ring-1 ${activeErrors.description ? "border-red-500/50 focus:border-red-500/60 focus:ring-red-500/20" : "border-white/10 focus:border-indigo-500/60 focus:ring-indigo-500/20"}`}
          />
          <FieldError message={activeErrors.description} />
        </div>
