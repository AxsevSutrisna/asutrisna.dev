...
const SocialLinks = () => {
 ...
 setLinks((data || []).length > 0 ? data : FALLBACK_SOCIAL_LINKS);
 setLoading(false);
};
 ...
 return String(left.display_name || left.displayName ||'').localeCompare(
 String(right.display_name || right.displayName ||''),
 );
});
}, [links]);

 const normalize = (item) => {
 const fallback = FALLBACK_SOCIAL_LINKS.find((link) => link.name === item.platform) || {};
 const iconKey = item.icon || fallback.icon ||'ExternalLink';

 return {
 name: item.platform || item.name || fallback.name ||'Social',
 displayName: item.display_name || item.displayName || fallback.displayName || item.platform,
 subText: item.sub_text || item.subText || fallback.subText ||'',
 ...
 const primary = orderedLinks.length > 0 ? normalize(orderedLinks[0]) : null;
 const others = orderedLinks.slice(1).map(normalize);
 ...
 >
 <div className={`absolute inset-0 opacity-0 group-hover:opacity-20 transition-opacity duration-500 bg-[#0f172a] ${link.gradient}`} />
 ...
 <div className="absolute inset-0 bg-[#0f172a] translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000" />
 </div>
 </a>
 );

 return (
 <div className="w-full bg-[#0f172a] rounded-2xl p-6 py-8 backdrop-blur-xl">
 <h3
 ...
 {loading && !primary ? (
 <div className="text-sm text-gray-500">Loading social links...</div>
 ) : null}
 ...
 data-aos-delay="100"
 >
 <div className={`absolute inset-0 opacity-0 group-hover:opacity-20 transition-opacity duration-500 bg-[#0f172a] ${primary.gradient}`} />
