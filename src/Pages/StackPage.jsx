...
import { Code, Database, Layout, Shield, Smartphone, Cpu, BrainCircuit, Server, Globe } from 'lucide-react';
import AOS from 'aos';
import 'aos/dist/aos.css';

const getCategoryConfig = (category) => {
  const cat = (category || 'General').toLowerCase();
  if (cat.includes('backend') || cat.includes('server')) return { icon: Server, color: 'text-emerald-400', bg: 'bg-emerald-400' };
  if (cat.includes('frontend') || cat.includes('ui')) return { icon: Layout, color: 'text-blue-400', bg: 'bg-blue-400' };
  if (cat.includes('ai') || cat.includes('machine learning')) return { icon: BrainCircuit, color: 'text-purple-400', bg: 'bg-purple-400' };
  if (cat.includes('devops') || cat.includes('security')) return { icon: Shield, color: 'text-amber-400', bg: 'bg-amber-400' };
  if (cat.includes('mobile')) return { icon: Smartphone, color: 'text-rose-400', bg: 'bg-rose-400' };
  if (cat.includes('iot') || cat.includes('electronic')) return { icon: Cpu, color: 'text-cyan-400', bg: 'bg-cyan-400' };
  if (cat.includes('database')) return { icon: Database, color: 'text-indigo-400', bg: 'bg-indigo-400' };
  if (cat.includes('web') || cat.includes('general')) return { icon: Globe, color: 'text-sky-400', bg: 'bg-sky-400' };
    ...
    >
      ...
      <div className="relative z-10 p-6">
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <h3 className="text-xl sm:text-2xl font-bold text-white tracking-tight">{category || 'General'}</h3>
        </div>
          ...
          {stacks.map((stack) => (
            ...
            >
              {stack.icon_url ? (
                ...
                <span className="text-xs text-gray-400 font-bold uppercase">{stack.name.substring(0, 2)}</span>
              )}
            </div>
          ))}
        </div>
...
export default function StackPage() {
  ...
  const groupAndSetStacks = (data) => {
    const grouped = data.reduce((acc, stack) => {
  ...
  return (
    <div className="flex flex-col min-h-screen" style={{ backgroundColor: 'var(--color-backdrop-base)' }}>
      <Navbar />
      
      <main className="flex-1 pt-24 pb-20 px-[5%] md:px-[10%] w-full overflow-hidden">
        <div className="text-center mb-16 pt-10" data-aos="fade-down" data-aos-duration="1000">
          <h1 className="text-4xl md:text-6xl font-display font-bold text-white mb-4">
            My <span style={{ color: 'var(--color-primary-light)' }}>Tech Stack</span>
          </h1>
          <p className="text-gray-400 max-w-2xl mx-auto text-base md:text-lg">
            A comprehensive look at the tools and technologies I use to architect solutions and empower communities.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 max-w-7xl mx-auto">
            {categories.map((category, index) => (
              <TechCategoryCard 
                ...
                animation={index % 3 === 0 ? "fade-up-right" : index % 3 === 1 ? "fade-up" : "fade-up-left"}
              />
            ))}
          </div>
        )}
