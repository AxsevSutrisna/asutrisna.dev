  ...
  ReactDOM.createPortal(
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-3 sm:p-6">
      ...
      >
        ...
        <div className="relative border border-white/12 rounded-2xl flex flex-col overflow-hidden shadow-2xl" style={{ backgroundColor: 'var(--color-backdrop-base)' }}>
...
const CourseForm = ({ initial, onSubmit, onCancel, submitLabel = "Save Course", uploading }) => {
  ...
  const inputCls = "cursor-target w-full bg-black/20 border border-white/10 rounded-xl px-4 py-2.5 text-gray-200 placeholder-gray-600 text-sm outline-none focus:border-white/20 transition-all";
    ...
    <form onSubmit={handleSubmit} className="p-5 sm:p-6 space-y-5" noValidate>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="sm:col-span-2">
          <label className={labelCls} style={{ color: 'var(--color-primary-light)' }}>Course Title *</label>
          <input
            type="text"
            value={form.title}
            onChange={set("title")}
        ...
        {/* Provider */}
        <div>
          <label className={labelCls} style={{ color: 'var(--color-primary-light)' }}>Provider *</label>
        ...
        {/* Date */}
        <div>
          <label className={labelCls} style={{ color: 'var(--color-primary-light)' }}>Completion Date</label>
          <div className="relative">
        ...
        {/* Link */}
        <div className="sm:col-span-2">
          <label className={labelCls} style={{ color: 'var(--color-primary-light)' }}>Certificate Link (URL)</label>
          <div className="relative">
        ...
        {/* Description */}
        <div className="sm:col-span-2">
          <label className={labelCls} style={{ color: 'var(--color-primary-light)' }}>Description</label>
          <textarea
        ...
        {/* Image */}
        <div className="sm:col-span-2">
          <label className={labelCls} style={{ color: 'var(--color-primary-light)' }}>Course Image / Badge</label>
      ...
      <div className="flex justify-end gap-2 pt-4 border-t border-white/5">
        ...
        <button type="submit" disabled={uploading} className="cursor-target relative group/btn">
          <div className="absolute -inset-0.5 rounded-xl opacity-60 blur group-hover/btn:opacity-100 transition duration-300" style={{ background: 'linear-gradient(90deg, var(--color-primary-dark), var(--color-primary-light))' }} />
          <div className="relative flex items-center gap-2 px-6 py-2.5 rounded-xl border border-white/10" style={{ backgroundColor: 'var(--color-backdrop-base)' }}>
            {uploading ? (
              ...
              <Upload className="w-4 h-4 text-indigo-400" style={{ color: 'var(--color-primary-light)' }} />
            )}
            <span className="text-sm font-medium text-gray-200">{uploading ? "Saving..." : submitLabel}</span>
          </div>
        </button>
      </div>
    </form>
...
const CourseCard = ({ course, onEdit, onDelete }) => {
  ...
  return (
    <div 
      ...
      style={{
        boxShadow: hovered ? '0 8px 32px -8px var(--color-primary-dark)' : 'none',
        borderColor: hovered ? 'var(--color-primary-dark)' : 'rgba(255,255,255,0.1)'
      }}
    >
      <div 
      ...
      <div className="flex-1 min-w-0">
        <h3 className="text-sm font-semibold text-white truncate group-hover:text-indigo-100 transition-colors" style={{ color: hovered ? 'var(--color-primary-light)' : 'white' }}>
          {course.title}
        </h3>
        <p className="text-xs text-gray-400 truncate mt-0.5">{course.provider}</p>
      ...
      <div className="flex items-center gap-2 opacity-60 group-hover:opacity-100 transition-opacity z-10">
        <button
          ...
          style={{ backgroundColor: hovered ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.05)' }}
        >
        ...
        <div className="flex items-center gap-4">
          <div className="relative shrink-0">
            ...
            <div className="relative w-10 h-10 rounded-xl border border-white/15 flex items-center justify-center" style={{ backgroundColor: 'var(--color-backdrop-base)' }}>
              <BookOpen className="w-5 h-5 text-indigo-400" style={{ color: 'var(--color-primary-light)' }} />
            </div>
          </div>
          <div>
      ...
      <div className="pt-2">
        {loading ? (
          ...
          <div className="relative rounded-2xl border border-white/8 border-dashed overflow-hidden">
            <div className="absolute inset-0 opacity-30" style={{ background: 'radial-gradient(ellipse at center, var(--color-primary-dark) 0%, transparent 70%)' }} />
            <div className="relative py-24 flex flex-col items-center gap-4">
              ...
              <div className="text-center">
                <p className="text-gray-300 font-medium text-sm">No courses found</p>
                <p className="text-gray-600 text-xs mt-1">Add your first course to show on your portfolio.</p>
              </div>
              <div className="mt-2">
                <AddNewButton onClick={() => setShowCreate(true)} label="Add First Course" />
              </div>
            </div>
          </div>
        ) : (
