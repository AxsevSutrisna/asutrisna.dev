...
const getInitials = (school = '') =>
  school.split(' ').slice(0, 2).map((w) => w[0]?.toUpperCase() ?? '').join('')
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="p-5 sm:p-6 space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="sm:col-span-2 space-y-1.5">
          ...
          <div className="relative">
            ...
            <input type="text" value={form.school} onChange={set('school')} placeholder="e.g. Universitas Teknologi Bandung" className={`${getInputClass('school')} pl-10`} />
          </div>
          <FieldError message={errors.school} />
        </div>
        <div className="space-y-1.5">
          <label className={labelCls}>Degree *</label>
          <input type="text" value={form.degree} onChange={set('degree')} placeholder="e.g. Bachelor of Science" className={getInputClass('degree')} />
          <FieldError message={errors.degree} />
        </div>
        <div className="space-y-1.5">
          <label className={labelCls}>Field of Study</label>
          <input type="text" value={form.field_of_study} onChange={set('field_of_study')} placeholder="e.g. Computer Science" className={getInputClass('field_of_study')} />
        </div>
        <div className="space-y-1.5">
          <label className={labelCls}>Grade / GPA</label>
          <input type="text" value={form.grade} onChange={set('grade')} placeholder="e.g. 3.8 / 4.0" className={getInputClass('grade')} />
        </div>
      </div>
      ...
      <div className="flex justify-end gap-2 pt-4 border-t border-white/5">
        ...
        <button type="submit" disabled={submitting} className="cursor-target relative group/s">
          ...
          <div className="relative flex items-center gap-2 px-6 py-2.5 rounded-xl border border-white/10" style={{ backgroundColor: 'var(--color-backdrop-base)' }}>
            {submitting ? <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" /> : <GraduationCap className="w-4 h-4 text-indigo-400" />}
            <span className="text-sm font-medium text-gray-200">{submitting ? 'Saving...' : (initial ? 'Update Education' : 'Save Education')}</span>
          </div>
        </button>
      </div>
    </form>
...
const EducationCard = ({ education, onEdit, onDelete, isLast }) => {
  ...
  const dateRange = formatDateRange(education.start_month, education.start_year, education.end_month, education.end_year, education.is_current)

  return (
    <div className="flex gap-4 sm:gap-6">
      {/* Timeline column */}
      <div className="flex flex-col items-center shrink-0">
        <div className={`relative w-10 h-10 rounded-xl border flex items-center justify-center text-xs font-bold shrink-0 transition-all duration-300 ${education.is_current ? 'bg-indigo-500/20 border-indigo-500/50 text-indigo-300' : 'bg-white/5 border-white/12 text-gray-400'} ${hovered ? 'scale-110 shadow-lg shadow-indigo-500/20' : ''}`}>
          ...
          <div className="p-5 flex flex-col gap-4">
            ...
            <div className="flex items-start justify-between gap-3 flex-wrap">
              <div className="min-w-0 space-y-1">
                ...
                <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-sm">
                  <span className="font-medium text-gray-300">{education.degree} {education.field_of_study ? `in ${education.field_of_study}` : ''}</span>
                </div>
              </div>
            </div>
            ...
            {education.activities && (
              <p className="text-sm text-gray-400 leading-relaxed line-clamp-2 border-l-2 border-white/8 pl-3 group-hover:border-indigo-500/30 transition-colors duration-300">
                <strong className="text-gray-300">Activities:</strong> {education.activities}
              </p>
            )}
            ...
            <div className="flex items-center justify-between gap-2 pt-2 border-t border-white/6">
              ...
              <div className="flex gap-2 opacity-60 group-hover:opacity-100 transition-opacity duration-200">
                ...
                <button type="button" onClick={() => onDelete(education.id)}
...
export default function Educations() {
  ...
  const fetchEducations = async () => {
    ...
    if (error) { console.error('Failed to fetch:', error); setEducations([]); setLoading(false); return }
    const normalized = (data || []).map(normalizeEducation).sort(compareEducationTimeline)
  ...
  const handleCreate = async (payload) => {
    ...
    if (error) { 
      ...
      alert(`Gagal menyimpan ke database: ${error.message || error.details || 'Unknown error'}`);
      return 
    }
    setShowCreate(false); fetchEducations()
  }

  const handleEdit = async (payload) => {
    if (!editEducation) return
    setSubmitting(true)
    const { error } = await supabase.from('educations').update(payload).eq('id', editEducation.id)
    setSubmitting(false)
    if (error) { 
      console.error('Failed to update:', error); 
      alert(`Gagal mengupdate ke database: ${error.message || error.details || 'Unknown error'}`);
      return 
    }
    setEditEducation(null); fetchEducations()
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-white tracking-tight">Educational History</h1>
            {!loading && (
              <div className="flex items-center gap-2 mt-1 flex-wrap">
                <span className="flex items-center gap-1 text-[11px] text-gray-400 bg-white/5 border border-white/8 rounded-full px-2.5 py-0.5">
                  <GraduationCap className="w-3 h-3" /> {educations.length} records
                </span>
                {currentCount > 0 && (
      ...
      {showCreate && (
        <Modal title="Add New Education" onClose={() => setShowCreate(false)}>
          <EducationForm onSubmit={handleCreate} onCancel={() => setShowCreate(false)} submitting={submitting} />
        </Modal>
      )}
      {editEducation && (
        <Modal title="Edit Education" onClose={() => setEditEducation(null)}>
          <EducationForm initial={editEducation} onSubmit={handleEdit} onCancel={() => setEditEducation(null)} submitting={submitting} />
        </Modal>
      )}
          ...
          <div className="space-y-0">
            {educations.map((edu, index) => (
              <EducationCard
                key={edu.id}
                education={edu}
                onEdit={(e) => { setEditEducation(e); setShowCreate(false) }}
