...
const mapExperienceFormErrors = (errors) => ({
   ...
   employment_type: errors.employment_type ? 'Pilih employment type yang valid.' : undefined,
   ...
   end_month: errors.end_month ? 'End month wajib diisi jika ini bukan current role.' : undefined,
   end_year: errors.end_year ? 'End year wajib diisi jika ini bukan current role.' : undefined,
})
...
const ExperienceForm = ({ initial = null, onSubmit, onCancel, submitting }) => {
   ...
   const handleSubmit = (e) => {
      ...
      const normalizedErrors = mapExperienceFormErrors(validateWorkExperienceForm(form))
      const nextErrors = Object.fromEntries(Object.entries(normalizedErrors).filter(([, value]) => Boolean(value)))
      ...
      const payload = buildWorkExperiencePayload({ ...form, tech_stack: techStack })
      onSubmit(payload)
   }
                     ...
                     <div className="flex flex-wrap gap-1.5">
                        {techStack.map((item) => (
                           <span key={item} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-200 text-xs">
                              {item}
                              <button type="button" onClick={() => removeTech(item)} className="cursor-target text-indigo-300/60 hover:text-white transition-colors">
                                 <X className="w-3 h-3" />
                              </button>
                           </span>
                        ))}
