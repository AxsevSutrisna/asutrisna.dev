...
const CertCard = ({ cert, onDelete, onView }) => {
  ...
  return (
    <div className="relative group/item">
      <div
        className="absolute -inset-0.5 rounded-2xl blur opacity-10 group-hover/item:opacity-30 transition duration-500"
        style={{
          background:
            "linear-gradient(90deg, var(--color-primary-dark), var(--color-primary-light))",
        }}
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">
            {loading
              ? "Loading..."
              : `Manage your ${certs.length} certificates and achievements`}
          </p>
        </div>
      </div>
      ...
      <Card>
        <div className="space-y-4">
          ...
          <label
            ...
            className={`cursor-target flex flex-col items-center justify-center w-full min-h-[200px] rounded-xl border-2 border-dashed cursor-pointer transition-all duration-300 ${
              dragOver
                ? "border-indigo-500/60 bg-indigo-500/10"
                : "border-white/10 bg-black/20 hover:border-indigo-500/40 hover:bg-black/40"
            }`}
          >
            {preview ? (
              <div className="relative p-4 w-full flex items-center justify-center">
                <img
                  src={preview}
                  alt="preview"
                  className="max-h-56 object-contain rounded-lg shadow-2xl border border-white/10"
                />
              </div>
            ) : (
          ...
          {file && (
            <div className="flex items-center justify-between gap-4 p-4 rounded-xl bg-white/5 border border-white/10 flex-wrap">
              ...
              <div className="flex gap-3 shrink-0 w-full sm:w-auto">
                ...
                >
                  <div
                    className="absolute -inset-0.5 rounded-lg opacity-60 blur group-hover/btn:opacity-100 transition duration-300"
                    style={{
                      background:
                        "linear-gradient(90deg, var(--color-primary-dark), var(--color-primary-light))",
                    }}
          ...
          <Card className="!p-16">
            <div className="text-center">
              ...
              <p className="text-gray-500 text-sm mt-1">
                Upload your first certificate above to showcase it on your
                portfolio.
              </p>
            </div>
          </Card>
