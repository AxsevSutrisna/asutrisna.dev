    ...
    >
      ...
      >
        <div className="flex items-center gap-4 min-w-0 flex-1">
          {/* Read Status Dot */}
          <div
            className={`w-2 h-2 rounded-full shrink-0 ${item.is_read ? "bg-transparent" : "bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.8)]"}`}
          />

          {/* Sender Info & Subject */}
          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-6 min-w-0 flex-1">
            <span
              className={`text-sm truncate w-full sm:w-48 shrink-0 ${item.is_read ? "text-gray-400 font-medium" : "text-white font-bold"}`}
            >
              {item.name}
            </span>
            <span
              className={`text-sm truncate min-w-0 flex-1 ${item.is_read ? "text-gray-500" : "text-gray-200 font-medium"}`}
            >
