...
export default function HeroContentDashboard() {
  ...
  const initForm = (heroData) => {
    setForm({
      ...
      typing_words: Array.isArray(heroData.typing_words)
        ? heroData.typing_words.join(",")
        : "",
      description: heroData.description,
      tech_badges: Array.isArray(heroData.tech_badges)
        ? heroData.tech_badges.join(",")
        : "",
      cta_buttons: Array.isArray(heroData.cta_buttons)
  ...
  const uploadImage = async (file) => {
    ...
    const { error: uploadError } = await supabase.storage
      .from("project-images")
      .upload(`hero-content/${fileName}`, file, {
        upsert: true,
      })
    ...
    const { data } = supabase.storage
      .from("project-images")
      .getPublicUrl(`hero-content/${fileName}`)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveStatus({
        type: "error",
        message: error.message || "Failed to save hero content",
      })
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-500 text-xs">
              Customize your portfolio home hero section
            </p>
          </div>
        </div>
      ...
      <div className="flex items-start gap-3 p-4 rounded-lg border border-blue-500/20 bg-blue-500/5 text-blue-100 text-sm">
        ...
        <p>
          This is your portfolio&apos;s hero section. All changes are
          automatically published to your home page in real-time.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div ref={formRef} className="lg:col-span-2 space-y-6">
          ...
          <div className="relative group/card">
            <div
              className="absolute -inset-0.5 rounded-2xl blur opacity-10 group-hover/card:opacity-20 transition duration-500"
              style={{
                background:
                  "linear-gradient(90deg, var(--color-primary-dark), var(--color-primary-light))",
              }}
          ...
          <div className="relative group/card">
            ...
            <div className="relative bg-white/5 backdrop-blur-xl border border-white/12 rounded-2xl p-6 space-y-5">
              ...
              <TagInput
                ...
                onChange={(value) =>
                  setForm((current) => ({ ...current, typing_words: value }))
                }
              ...
              <TagInput
                ...
                onChange={(value) =>
                  setForm((current) => ({ ...current, tech_badges: value }))
                }
              ...
              {form.cta_buttons && form.cta_buttons.length > 0 ? (
                <div className="space-y-4">
                  {form.cta_buttons.map((cta, idx) => (
                    <div
                      key={idx}
                      className="relative group/item p-5 rounded-xl bg-black/20 border border-white/5 space-y-4 hover:border-white/10 transition-colors"
                    >
                        ...
                        {form.cta_buttons.length > 1 && (
                          <button
                            ...
                            onClick={() => {
                              setForm((current) => ({
                                ...current,
                                cta_buttons: current.cta_buttons.filter(
                                  (_, i) => i !== idx
                                ),
                              }))
                            }}
                ...
                <div className="p-8 rounded-xl bg-black/20 border border-dashed border-white/10 text-center flex flex-col items-center justify-center gap-2">
                  ...
                  <p className="text-gray-600 text-xs">
                    Click the + Add CTA button to create one.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="relative group/card">
            ...
            <div className="relative bg-white/5 backdrop-blur-xl border border-white/12 rounded-2xl p-6 space-y-5">
              ...
              <div>
                <label className="text-xs text-indigo-300/70 uppercase tracking-wider font-medium mb-2 block">
                  Hero Image
                </label>
                <label className="cursor-target flex items-center gap-4 w-full bg-black/20 border border-dashed border-white/15 rounded-xl px-4 py-6 cursor-pointer hover:border-indigo-500/40 hover:bg-black/40 transition-all">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      PNG, JPG, WEBP, GIF (recommended: square or wide aspect)
                    </p>
                  </div>
                  <input
                    type="file"
              ...
              <InputField
                ...
                onChange={set("hero_image_alt")}
                placeholder="Developer illustration"
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-6 rounded-2xl border border-white/12 bg-white/5 backdrop-blur-xl p-6 space-y-4">
            ...
            <div className="rounded-lg overflow-hidden border border-white/10 bg-[#0f172a] ] ]">
              ...
              <div className="p-4 space-y-3">
                ...
                <h3 className="text-lg font-bold text-white leading-tight">
                  ...
                  <span className="bg-[#0f172a] text-white">
                ...
                <div className="flex flex-wrap gap-1.5 pt-2">
                  {normalizeList(form.tech_badges)
                    .slice(0, 3)
                    .map((tech) => (
                      <span
                        key={tech}
                        className="px-2 py-1 rounded-full bg-white/5 border border-white/10 text-[10px] text-gray-300"
                      >
                        {tech}
                      </span>
                    ))}
                ...
                <div className="flex gap-2 pt-2 flex-wrap">
                  ...
                  {form.cta_buttons && form.cta_buttons.length > 3 && (
                    <span className="px-3 py-1.5 text-[10px] text-gray-400">
            ...
            <div className="text-[11px] text-gray-400 space-y-1">
              <p>
                💡 Changes are automatically reflected on your home page in
                real-time.
              </p>
            </div>
          </div>
