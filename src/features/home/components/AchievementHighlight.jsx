...
export default function AchievementHighlight() {
  ...
  const getGridClasses = (index, totalItems) => {
    ...
    if (totalItems === 2) return index === 0 ? 'md:col-span-2 lg:col-span-2 lg:row-span-2' : 'col-span-1 lg:col-span-1 lg:row-span-2';
    if (totalItems === 3) return index === 0 ? 'md:col-span-2 lg:col-span-2 lg:row-span-2' : 'col-span-1 lg:col-span-1 lg:row-span-1';
    if (totalItems === 4) {
    ...
    return index === 0 ? 'md:col-span-2 lg:col-span-2 lg:row-span-2' : 'col-span-1 lg:col-span-1 lg:row-span-1';
  };
      ...
      <div className="container mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
            <div>
              <h2 className="text-3xl sm:text-4xl lg:text-5xl font-display font-bold text-white leading-tight">
                My <span style={{ color: 'var(--color-primary-light)' }}>Achievements</span>
              </h2>
              <p className="mt-2 text-sm sm:text-base" style={{ color: 'var(--color-text-muted)' }}>
                Milestones, competitive awards, and honors I've received along my journey.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 lg:auto-rows-[minmax(0,1fr)] gap-5 animate-cards-in">
          {highlighted.map((item, index) => {
            ...
            const isLarge = gridClasses.includes('col-span-2') || gridClasses.includes('col-span-3');
                ...
                >
                  ...
                  <div className="flex items-start justify-between">
                    <div
                      ...
                      aria-hidden="true"
                    >
                      <Trophy className={isLarge ? 'w-6 h-6 lg:w-8 lg:h-8' : 'w-5 h-5'} />
                    </div>
                    <span
                  ...
                  <div className="space-y-1 flex-1 flex flex-col justify-center mt-2 lg:mt-0">
                    ...
                    {item.deskripsi && (
                      <p className="text-xs lg:text-sm mt-2 italic border-l-2 pl-3" style={{ color: 'var(--color-text-muted)', borderColor: 'var(--color-primary-light)' }}>
                        "{item.deskripsi}"
                      </p>
                    )}
                  </div>
                  ...
                  <div className={`flex flex-wrap items-center justify-between gap-3 border-t border-white/[0.06] ${
