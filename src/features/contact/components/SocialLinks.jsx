...
const SocialLinks = () => {
  ...
  const normalize = (item) => {
    const fallback =
      FALLBACK_SOCIAL_LINKS.find((link) => link.name === item.platform) || {}
  ...
  const renderCard = (link, index) => (
    ...
    >
      <div
        className={`absolute inset-0 opacity-0 group-hover:opacity-20 transition-opacity duration-500 bg-[#0f172a] ${link.gradient}`}
      />
    ...
    <div className="w-full bg-[#0f172a] rounded-2xl p-6 py-8 backdrop-blur-xl">
      ...
      <div className="flex flex-col gap-4">
        ...
        {primary ? (
          ...
          >
            <div
              className={`absolute inset-0 opacity-0 group-hover:opacity-20 transition-opacity duration-500 bg-[#0f172a] ${primary.gradient}`}
            />
        ...
        {others.length > 4 && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {others
              .slice(4, 6)
              .map((link, index) => renderCard(link, index + 4))}
          </div>
        )}
      </div>
    </div>
